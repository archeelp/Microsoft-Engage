...
const gradeSubmission = async (req, res) => {
  try {
    await db.Submission.findOneAndUpdate({_id: req.params.submissionId, assignment: req.params.assignmentId}, {$set: {grade: req.body.grade}});
    res
      .status(200)
      .json({ message: "Submission Graded Successfully" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: error.message });
  }
